...
#==============================================================================
            ...
            elif msg["action"] == "send rsa":
                from_name = self.logged_sock2name[from_sock]
                print(msg)
                rsa_public_key =  msg["rsa_public_key"]
                rsa_encryption_alpha = msg["encryption_alpha"]
                the_guys = self.group.list_me(from_name)
                for g in the_guys[1:]:
                    to_sock = self.logged_name2sock[g]
                    mysend(to_sock,json.dumps({"action":"recieve rsa", "rsa_public_key": rsa_public_key, "from": from_name, "encryption_alpha": rsa_encryption_alpha}))
                mysend(from_sock,json.dumps({"action":"sent rsa", "results":"success"}))
            
            elif msg["action"] == "recieve rsa":
                rsa_public_key = msg["rsa_public_key"]
                rsa_encryption_alpha = msg["encryption_alpha"]
                the_guys = self.group.list_me(from_name)
                for g in the_guys[1:]:
